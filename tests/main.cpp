...
cout<<"FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF\n";    
    ...
    for (int j=1;j<=6;j++){  
        for (int i=1;i<=6;i++){
        Phi(i, j) = yPhi(6 * (j - 1) + i + 6);
		}
    }
    cout<<"XX\n"; 
	cout<<"Y_old"<<endl<<Y_old<<endl;
 ...
 cout<<"GGGGG\n";   
    ...
    Matrix dAdY = union_vector(transpose(dAds*LT*U),zeros(3));
    
    // Measurement update
    auto[K, Yaux, Paux] = MeasUpdate ( Y, obs(i,2), Azim, sigma_az, dAdY, P, 6 );
    Y=Yaux;
	P=Paux;
    ...
    Matrix dEdY = union_vector(transpose(dEdsaux*LT*U),zeros(3));
    
    // Measurement update
    auto [K2, Yaux2, Paux2] = MeasUpdate ( Y, obs(i,3), Elevaux, sigma_el, dEdY, P, 6 );
    Y=Yaux2;
	P=Paux2;
    // Range and partials
    r = extract_vector(Y,1,3);
    ...
    Matrix dDdY = union_vector(transpose(dDds*LT*U),zeros(3));
    
    // Measurement update
    auto [K3, Yaux3, Paux3] = MeasUpdate ( Y, obs(i,4), Dist, sigma_range, dDdY, P, 6 );
	Y=Yaux3;
	P=Paux3;
	}
cout<<"HHHHH\n";	
