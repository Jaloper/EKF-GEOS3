...
//
/** @file DEInteg.cpp
 *  @brief Solves a system of differential equations using an adaptive integration method with error control.
 *
...
Matrix DEInteg(Matrix func(double t, Matrix y), double t,double tout,double relerr,double abserr,int n_eqn,Matrix& yParametro){;
	...
	double temp1, hi, hold, hnew, absh, term, psijm1, gamma, eta, p5eps,round, sum, tau, xold, err, rho_aux, erkp1, r;
	 bool phase1, nornd,crash, success;
	 int k, kold, ki, iAux, ifail;
	 int kp1;
	  int kp2;
	  int km1;
	  int km2;	
	...
	while (true){   // Start step loop
	  ...
	  if (fabs(x-t) >= absdel){
		  ...
		  for(int j = 1; j <= ki; j++){
			  ...
			  ypout = ypout + extract_column(phi,iAux+1)*rho(iAux+1);
		  }	
		  yout = y + yout*hi;
		  y    = yout;
